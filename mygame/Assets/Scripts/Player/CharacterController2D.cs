...
public class CharacterController2D : MonoBehaviour
{
    ...
    [Tooltip("Combination time")]
    [SerializeField] float maxComboDelay = 0.4f;
    [Tooltip("Attack button interval time")]
    [SerializeField] float slashIntervalTime = 0.2f;

    [Header("Attack numerical parameters")]
    ...
    private void UpdateVelocity()
    {
        if (!data.GetDeadStatement())
        {
            ...
            animator.SetFloat(animatorVelocitySpeed, controllerRigidbody.linearVelocity.y);

            if (canMove && gameManager.IsEnableInput())
            {
                controllerRigidbody.linearVelocity = new Vector2(vectorInput.x * maxSpeed, velocity.y);
    ...
    private void UpdateDirection()
    {
        if (canMove && !data.GetDeadStatement())
        {
            ...
            if (controllerRigidbody.linearVelocity.x > 1 && isFacingLeft)
            {
                isFacingLeft = false;
                transform.localScale = flippedScale;
            }
            else if (controllerRigidbody.linearVelocity.x < -1 && !isFacingLeft)
            {
                isFacingLeft = true;
    ...
    IEnumerator GrabWallJump()
    {
        ...
        controllerRigidbody.linearVelocity = new Vector2(transform.lossyScale.x * 
    ...
    {
        controllerRigidbody.linearVelocity.Set(controllerRigidbody.linearVelocity.x, 0);
    ...
    public IEnumerator TakeDamage(Enemy enemy)
    {
        ...
        if (!data.GetDeadStatement())
        {
            ...
            if (isFacingLeft)
            {
                controllerRigidbody.linearVelocity = new Vector2(-1f, 1f) * hurtForce;
            }
            else
            {
                controllerRigidbody.linearVelocity = new Vector2(1f, 1f) * hurtForce;
